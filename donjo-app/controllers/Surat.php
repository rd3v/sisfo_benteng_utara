...
class Surat extends Admin_Controller {
	...
	public function form($url = '',$id_penduduk = "",$stop = false,$clear = '')
	{
		...
		if (!empty($_POST['nik']))
		{
			...
			$data['individu'] = $this->surat_model->get_penduduk($id_penduduk);
			$data['anggota'] = $this->keluarga_model->list_anggota($data['individu']['id_kk']);
			$this->db->where('nik',$data['individu']['nik'])->update('tweb_surat_online',['read' => 1]);
		}
		else
		{
			$data['individu'] = NULL;
